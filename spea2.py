...
class Solution:
    ...
    def copy(self):
        ...
        new_sol.strength = self.strength
        new_sol.raw_fitness = self.raw_fitness
        new_sol.density = self.density
...
class SPEA2Solver:
    ...
    def calculate_fitness(self, population):
        ...
        k = int(sqrt(len(population)))
        for i in population:
            distances = []
            for j in population:
                if i != j:
                    d = sqrt(sum((a - b)**2 for a, b in zip(i.objectives, j.objectives)))
                    distances.append(d)
            distances.sort()
            density[i] = 1 / (distances[k] + 2)

        for sol in population:
            sol.strength = strength[sol]
            sol.raw_fitness = raw_fitness[sol]
            sol.density = density[sol]
            sol.fitness = sol.raw_fitness + sol.density

    def environmental_selection(self, combined):
        new_archive = [sol for sol in combined if sol.raw_fitness < 1]
        
        if len(new_archive) < self.pop_size:
            combined_sorted = sorted(combined, key=lambda x: x.fitness)
            fill_count = self.pop_size - len(new_archive)
            new_archive += [sol.copy() for sol in combined_sorted if sol not in new_archive][:fill_count]
        elif len(new_archive) > self.pop_size:
            self.truncate_archive(new_archive)
        
        return new_archive[:self.pop_size]
        ...
        while len(archive) > self.pop_size:
            ...
            for i in range(len(archive)):
                ...
                for j in range(len(archive)):
                    if i != j:
                        d = sqrt(sum((a - b)**2 for a, b in zip(archive[i].objectives, archive[j].objectives)))
                        distances.append(d)
                distances.sort()
                k = int(sqrt(len(archive)))
                current_dist = distances[k-1]
        ...
        for gen in range(self.max_gen):
            ...
            while len(offspring) < self.pop_size:
                if len(self.archive) >= 2:
                    parents = random.sample(self.archive, 2)
    ...
    def calc_hypervolume(self):
        if not self.archive:
            return 0
        ref = [max(sol.objectives[0] for sol in self.archive) * 1.1,
               max(sol.objectives[1] for sol in self.archive) * 1.1]
        
        sorted_pop = sorted(self.archive, key=lambda x: x.objectives[0])
        hv = 0
        prev = [0, ref[1]]
    ...
    def plot_results(self):
        ...
        plt.subplot(131)
        plt.scatter([s.objectives[1] for s in self.archive], 
...
if __name__ == "__main__":
    ...
    solver.archive.sort(key=lambda x: x.objectives[0])
    for sol in solver.archive[:5]:
